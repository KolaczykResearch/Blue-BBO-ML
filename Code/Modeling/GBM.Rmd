...
GBM_C <- function(folds = 10, seeds = 2023){
  ...
  return(list(gbm.rmse, validate.pred))
}
...
```{r}
validate.pred_c = matrix(unlist(GBM_results_c[2,]),ncol=8,byrow = TRUE)
validate.pred_f = matrix(unlist(GBM_results_f[2,]),ncol=8,byrow = TRUE)
...
a = caret::RMSE(validate_c_y, validate.pred_c)
b = caret::RMSE(validate_f_y, validate.pred_f)
print(c(a, b))
```